...
datasets = ['freesolv', 'qm9', 'herg','esol' , 'lipo' ,\
            'sider' , 'tox21' , 'clintox' , 'bace' , 'bbbp' ]
...
if __name__ == '__main__':
    ...
    del labels
    del adj_mats
    xx= rand(int(avg_node), int(avg_node), density=avg_edge/(avg_node * avg_node), format='csr')
    xx.data[:] = 1
    ...
    if args.model == 'graphsage':
        ...
        model.to(device)
        f = [torch.randn(int(avg_node)),torch.randn((int(avg_node), int(feat_dim/4))),torch.randn((int(avg_node)*2, int(feat_dim/4))) ]
        f = [level.to(device=device, dtype=torch.long, non_blocking=True) for level in f]
        prepare_input = lambda res:  { "feature_matrix" : torch.randn(int(avg_node), feat_dim).to(device=device, dtype=torch.float32, non_blocking=True) ,\
                                      "forest" : f }
        ...
        prepare_input = lambda res:  { "adj_matrix" : torch.from_numpy(xx.toarray()).to(device=device, dtype=torch.float32, non_blocking=True) ,\
                                       "feature_matrix" : torch.randn((int(avg_node), feat_dim)).to(device=device, dtype=torch.float32, non_blocking=True) }
        model.to(device)
        flops, params = get_model_complexity_info(model,  input_res=(1, 1,1), 
    ...
    macs, params = get_model_complexity_info(model,  input_res=(1, 1,1), 
                                              input_constructor=prepare_input,as_strings=False,
                                              print_per_layer_stat=True, verbose=True)
    print('{:<30}  {:<8}'.format('Number of parameters: ', params))
